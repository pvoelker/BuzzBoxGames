...
namespace BuzzBoxGamesApp.Behaviors
{
    public partial class TicTacToeInvalidMove : Behavior<ContentPage>
    {
        ...
        public static readonly BindableProperty SoundNameProperty =
            BindableProperty.Create(nameof(SoundName), typeof(string), typeof(TicTacToeInvalidMove), propertyChanged: async (b, o, n) =>
            {
                if (b is TicTacToeInvalidMove bb)
                {
        ...
        protected override void OnAttachedTo(ContentPage control)
        {
            ...
            if (context != null)
            {
                context.InvalidMove = new AsyncRelayCommand(async () =>
                {
